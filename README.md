...
- [x] `tools/vector_store.py`：Chroma 建 CEA 标准租约知识库（PR #15，5 份 CEA 模板可一键 ingest）
- [ ] 条款提取：押金 / 提前终止 / 维修责任 / 水电分担（B/contract-agent 长寿分支进行中）
- [ ] 与 CEA 标准对比，标记偏离项
- [ ] 输出风险条款列表 + 严重程度评分
- [ ] `tests/test_contract.py`：用 sample_contract（D 提供的 docx）验证检出预设陷阱
...
- [ ] `agents/risk_agent.py` 真实逻辑（目前是 placeholder）
- [ ] 读取 `data/cea_agents.csv` 离线兜底（API 限流时用）
- [ ] LLM 生成风险评估总结
- [ ] 输出注册状态 + 风险评分 + 建议
- [ ] `tests/test_risk.py`：已注册中介名验证返回"已注册"

**Day 4 Checkpoint**
- [x] pytest 16/16 全过（intake / location / orchestrator / main_runner）
- [x] 4 个 Agent 输出统一符合 `AgentOutput` schema（A 在 `agents/__init__.py` 定义）
- [ ] B / C / D 的真实 Agent 单测（目前只有 A 的 location 是真实逻辑）
...
**端到端演示**
- [x] [全员] Jurong West 用例跑完整流程（B 实测 `adk web adk_apps`，5 个 LLM 调用串通）
- [ ] [全员] 报告完整：通勤 ✅ + 租金（C placeholder）+ 合同（B placeholder）+ 中介（D placeholder）+ 议价
- [ ] [B] Guardrail 拦截至少 1 个 PII + 1 个 prompt injection

**Day 5 Checkpoint ★**
- [x] 端到端演示 < 3 分钟（实测 fallback 链路约 30 秒）
- [ ] **`git tag baseline-v1` 锁版本**（等 4 个 agent 全部上真实逻辑后再打）
