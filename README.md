...
SafeNest 是一个面向新加坡租房场景的多智能体助手。用户输入目标地址、预算、合同 PDF，系统并行调度 4 个专业 Agent，输出一份覆盖 **通勤 / 租金 / 合同 / 中介风险** 的综合报告，并附带可执行的议价建议。
...
- [ ] [全员] (polish) 把 synthesizer 现有的议价建议升级为完整邮件草稿,如果有时间再做

**Day 5 Checkpoint ★**
- [x] 端到端演示 < 3 分钟（fallback 链路 30 秒；LLM 通畅时 10-20 秒）
- [ ] **`git tag baseline-v1` 锁版本**（4 agent 全部真实化已达成，等 Guardrail 接好再打 tag）
...
- [x] Prompt injection 攻击用例:已在 [tests/test_guardrails.py](tests/test_guardrails.py) 实现 10 个 case(单独文件不再需要)
